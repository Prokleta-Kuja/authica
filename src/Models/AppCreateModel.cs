...
public class AppCreateModel
{
    ...
    public Dictionary<string, string>? Validate(IApps translation, HashSet<string> names, int minPassLength, int maxPassLength)
    {
        ...
        if (!string.IsNullOrWhiteSpace(Secret))
            if (Secret.Length < minPassLength)
                errors.Add(nameof(Secret), translation.ValidationTooShort(minPassLength));
            else if (Secret.Length > maxPassLength)
                errors.Add(nameof(Secret), translation.ValidationTooLong(maxPassLength));
