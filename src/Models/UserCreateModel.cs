...
public class UserCreateModel
{
    ...
    public Dictionary<string, string>? Validate(IUsers translation, HashSet<string> emails, HashSet<string> usernames, int minPassLength, int maxPassLength)
    {
        ...
        if (!string.IsNullOrWhiteSpace(Password))
            if (Password.Length < minPassLength)
                errors.Add(nameof(Password), translation.ValidationTooShort(minPassLength));
            else if (Password.Length > maxPassLength)
                errors.Add(nameof(Password), translation.ValidationTooLong(maxPassLength));
